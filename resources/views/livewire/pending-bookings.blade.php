      ...
      <div class="row">
        <div class="col-md-12">
            ...
            <div class="card-header">
                <h3 class="card-title">Pending Bookings Records for {{session()->get('pano_client_data.FullNames')}} </h3>
            </div>
            </div>
...
<table id="example" class="table table-striped table-bordered">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td><a href="{{route('PendingBookings',['bookingslug'=>$bookings->IDs])}}"><button class="btn btn-danger" > Cancel</button></a></td>
            
            {{--<td>{{$bookings->Debit}}</td>
            <td>{{$bookings->Credit}}</td>
            <td>{{$bookings->Branch}}</td>--}}
        </tr>
        @endforeach

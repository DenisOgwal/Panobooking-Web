    ...
    <div class="container">
        <div class="row">
            <div class="col-md-8 col-md-offset-2 text-center heading-section animate-box">
                ...
                <p>{{$carsearched->count()}} Results for {{request()->input('pickuplocation')}} Cars Search</p>
            </div>
        </div>
        <div class="row">
            @foreach ($carsearched as $carsearch)
            <div class="col-md-4 col-sm-6 fh5co-tours animate-box" data-animate-effect="fadeIn">
                <div href="{{route('CarDetails',['carslug'=>$carsearch->IDs])}}"><img src="https://panobooking.com/AndroidDriver/FileUpload/{{$carsearch->DriverName}}{{$carsearch->TaxiNo}}.jpg"  class="img-responsive" onerror="this.onerror=null;this.src='images/image-placeholder.png';"/>
                    <div class="desc">
                        <span></span>
                        <h3>{{$carsearch->TaxiNo}}</h3>
                        <span class="price">{!!$currencypick!!}. {{number_format(ceil($carsearch->Price/$currenyvalue),0)}}</span>
                        <a class="btn btn-primary btn-outline" href="{{route('CarDetails',['carslug'=>$carsearch->IDs])}}">Book Now <i class="icon-arrow-right22"></i></a>
                    </div>
                </div>
            </div>   
            @endforeach      

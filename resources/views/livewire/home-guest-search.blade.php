    ...
    <div class="container">
        <div class="row">
            <div class="col-md-8 col-md-offset-2 text-center heading-section animate-box">
                <h3>Home Guest House Search Results</h3>
                <p>{{$homeguestsearched->count()}} Results for {{request()->input('Destination')}} Home Guest Houses Search</p>
            </div>
        </div>
        <div class="row">
            @foreach ($homeguestsearched as $homeguestsearch)
            <div class="col-md-3 col-sm-6 fh5co-tours animate-box" data-animate-effect="fadeIn">
                <div href="{{route('HomeGuestSingle',['homeguestslug'=>$homeguestsearch->IDs])}}"><img src="https://panobooking.com/AndroidFiles/GuestHouseImages/{{$homeguestsearch->GuestHouseName}}.jpg" alt="{{$homeguestsearch->GuestHouseName}}  Image" class="img-responsive" onerror="this.onerror=null;this.src='images/image-placeholder.png';"/>
                    <div class="desc">
                        <span></span>
                        <h3>{{$homeguestsearch->GuestHouseName}}</h3>
                        <span>{{$homeguestsearch->City}}</span>
                        <span class="price">UGX {{$homeguestsearch->Prices}}</span>
                        <a class="btn btn-primary btn-outline" href="{{route('HomeGuestSingle',['homeguestslug'=>$homeguestsearch->IDs])}}">Book Now <i class="icon-arrow-right22"></i></a>
                    </div>
                </div>
            </div>   
            @endforeach
